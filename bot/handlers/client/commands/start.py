...
async def start_about(message: types.Message, state: FSMContext):
    if len(message.text) < 300:
        ...
        if existed == "yes":
            ...
            await send_msg_user(message.from_user.id, 
                                "Теперь расскажем подробнее о боте 😊\n\nКак уже говорилось, бот помогает случайным образом находить тебе компанию для кофе раз в две недели, чтобы познакомиться с новыми людьми или просто приятно провести время.\n\nПри распределении ты получишь свой смайл, например, 🐘, а также смайлы от 0 до 2 других участников. Смайл — это как приватный ключ и подтверждение того, что именно ты выпал другому человеку в Random Coffee. Поэтому ты можешь написать человеку только его смайл, и он сразу поймёт, по какому поводу ты пишешь!")
            await send_msg_user(message.from_user.id, 
