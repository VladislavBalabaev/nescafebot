...
async def start_email_set(message: types.Message, state: FSMContext):
    ...
    if message.text.replace(' ', '') == email_code:
        ...
        await send_msg_user(message.from_user.id, 
                            "Код неверный)\nДавай заново\n\nP.S. Если в первый раз была написана не своя почта, ничего страшного, просто введи /cancel и заново /start",
                            fail=True)
...
async def start_about(message: types.Message, state: FSMContext):
    if len(message.text) < 300:
        ...
        if existed == "yes":
            ...
            await send_msg_user(message.from_user.id, 
                                "Теперь чуть подробнее расскажем о боте)\n\nКак уже говорилось, суть бота в том, чтобы раз в две недели подбирать случайным образом компаньона на кофе, для того, чтобы познакомиться или просто приятно провести время.\n\nВо время распределения придет твой смайл, например, \U0001F418, а также от нуля до двух пользователей с их смайлами.\n\n Смайл как приватный ключ, гарантия того, что действительно ты выпал(а) другому человеку при random coffee. Поэтому, ты можешь написать человеку только лишь его смайл и он сразу поймет, по какому поводу ты пишешь)")
            await send_msg_user(message.from_user.id, 
