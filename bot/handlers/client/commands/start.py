...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not exist:
        await send_msg_user(message.from_user.id, 
                            "Привет! 😊\nДобро пожаловать в бот Random Coffee для студентов и выпускников РЭШ, созданный студентами MAE'25 @vbalab и @Madfyre.\n\nВсё очень просто: раз в две недели, с учётом твоего чёрного списка, мы случайным образом подбираем тебе других рэшевцев для встречи за кофе.\n\nДавай начнём с регистрации, это займёт минуту!")

        await send_msg_user(message.from_user.id,
                            "Для того, чтобы подтвердить, что ты студент РЭШ, пожалуйста, укажи свой адрес электронной почты @nes.ru [если нет такой возможности, напиши админам - @vbalab и @Madfyre]")
